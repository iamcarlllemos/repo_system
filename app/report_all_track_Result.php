...
<?php
  ...
  $GET_MEKUS =$_GET['search'];
function generateRow($conn){
  if(isset($_GET['search'])){
    $search =$_GET['search'];
    $contents = '';
    $sql = "SELECT * FROM tbl_storage WHERE TRACK ='".$search."' ORDER BY VIEW_COUNTER DESC";
...
class MYPDF extends TCPDF {
  ...
  public function Footer() {
    ...
    $pdf->SetTitle('LIST OF RECORDS- '.$GET_MEKUS);  
    ...
    <table cellpadding="2" width="100%" border="1" style="border-collapse: collapse">
      <tr>
        <th colspan="6">Track: '.$GET_MEKUS.'</th>
      </tr>
			<tr>
				<th>TITLE</th>
				<th>PUB YEAR</th>
