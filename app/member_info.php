...
if(isset($_GET['q'])){
  ...
  $sql = "SELECT * FROM tbl_member tr WHERE tr.ID= '$q'";
    ...
    $row = $query->fetch_assoc();
      ...
      $STUDENT_NO     =htmlentities($row['STUDENT_NO']);
      $MNAME         	=htmlentities($row['LASTNAME'].', '.$row['FIRSTNAME'].' '.$row['MI']);
      $GENDER   			=htmlentities($row['GENDER']);
      $ADDRESS    		=htmlentities($row['ADDRESS']);
      $CONTACT  			=htmlentities($row['CONTACT']);
      $PROFILE        =htmlentities($row['PROFILE']);
      $EMAIL    			=htmlentities($row['EMAIL']);
      $ROLE    			  =htmlentities($row['ROLE']);
      $ACC_STATUS   	=htmlentities($row['ACC_STATUS']);
      $DATE_CREATED   =htmlentities($row['DATE_CREATED']);
        ...
        <div class="col-md-5">
              ...
              <div class="card-body box-profile">
                <div class="text-center">
                 <?php 
                  ...
                  <?php }else{ ?>
                    <img src="data:image/jpg;charset=utf8;base64,<?=base64_encode($row['PROFILE']); ?>" width="150" height="150" class="img-circle">
                  <?php }?>
                ...
                <li class="list-group-item">
                    <b><i class="fa fa-sharp fa-solid fa-phone-office mr-1"></i> ID NUMBER</b> <a class="float-right"><?=$STUDENT_NO;?></a>
                  </li>
                  <li class="list-group-item">
                    <b><i class="fa fa-solid fa-circle-info mr-1"></i> STATUS </b> <a class="float-right">
                    <?php 
                    if ($row['ACC_STATUS'] ==0){
                      echo '
                      <i class="fas fa-solid fa-circle-xmark text-danger"></i> DEACTIVE 
                      ';
                    }elseif($row['ACC_STATUS']==1){
                      echo '<i class="fas fa-solid fa-circle-check text-success"></i> ACTIVE';
                    }
                    ?>
                    </a>
                  </li>
            ...
            <div class="card mt-3">
                <div class="card-header">
                  <h5 class="card-title m-0">Projects Posted</h5>
                </div>
                <div class="card-body scroll">
                    <?php 
                      $sql2 = "SELECT * FROM tbl_storage WHERE POST_BY_ID = '$q'";
                      $query2 = $conn->query($sql2);
                      $count = 0;
                      if($query2->num_rows > 0) {
                      while($row2 = $query2->fetch_assoc()) {
                        $count = $count+=1;
                        $uid = base64_encode($row2['ID']);
                    ?>
                      <ul class="list-unstyled">
                        <li class="list-unstyled-item">
                          <a href="home-projects-details.php?news=<?= $uid  ?>" class="nav-link text-dark p-0"><?php echo $count . '). ' . $row2['POST_TITLE'] ?></a>
                        </li>
                      </ul>
                    <?php
                      }
          ...
          <div class="col-md-7">
            <div class="card">
              ...
              <table id="customers" class="table table-bordered text-nowrap">
                  ...
                  <?php }else{ ?>
                    <img src="data:image/jpg;charset=utf8;base64,<?=base64_encode($row['ATTACHED_ID']); ?>" width="100%" height="100%" class="img-thum elevation-3">
                  <?php }?>
					</td>
                  </tr>       
                  <tr>
